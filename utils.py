    ...
    with torch.no_grad():
        ...
        with tqdm(test_loader, mininterval=5.0, maxinterval=50.0) as it:
            for batch_no, test_batch in enumerate(it, start=1):
                ...
                # --- DEBUG: RUN ONLY ON FIRST BATCH ---
                # if batch_no == 1:
                #     print("==== DIFFUSION DEBUG ====")
                #     print("eval_points sum:", eval_points.sum().item())
                #     print("c_target WVHT min/max:",
                #           c_target[:,:,2].min().item(),
                #           c_target[:,:,2].max().item())
                #     print("samples WVHT range:",
                #           samples[:,:,:,2].min().item(),
                #           samples[:,:,:,2].max().item())
                #     print("scaler WVHT:", scaler[2].item(),
                #           "mean WVHT:", mean_scaler[2].item())
                #     print("c_target shape:", c_target.shape)
                #     print("samples shape:", samples.shape)
                #     print("eval_points shape:", eval_points.shape)
                #     print("Unique values in c_target WVHT:",
                #           torch.unique(c_target[:,:,2]))
                #     exit()
            ...
            # Calculate final metrics with safety check
            if evalpoints_total > 0:
                final_rmse = np.sqrt(mse_total / evalpoints_total)
                final_mae = mae_total / evalpoints_total
            else:
                print("WARNING: No evaluation points found! Check gt_mask setup.")
                final_rmse = 0.0
                final_mae = 0.0
